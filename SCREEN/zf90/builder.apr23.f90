...
! need to get the points on the sphere and their weights
  ...
  allocate( ure( npt, ibl : ibh ), uim( npt, ibl : ibh ) )
...
! open up the enkfile, read through while in kpt loop
  open(unit=20,file='enkfile',form='formatted',status='old')
  rewind(20)
...
!
!
  open(unit=21,file='listwfile',form='formatted',status='old')
  rewind(21)
...
!!!  do ik2=1,nk2
  do ik1=1,nk1
!  qin(1) = 1.0/(8.0*dble(nk1))+dble(ik1-1)/dble(nk1)
!   qin(1) = 0.5d0/dble(nk1) + dble( ik1 -1) / dble(  nk1 )
!   qin(1) = 1.0/(8.0)+dble(ik1-1)/dble(nk1)
    qin(1) = ( kshift(1) + dble( ik1 - 1 )) / dble( nk1 )
  do ik2=1,nk2
!  qin(2) = 2.0/(8.0*dble(nk2))+dble(ik2-1)/dble(nk2)
!   qin(2) = 0.5d0/dble(nk2) + dble( ik2 -1) / dble( nk2 )
!  qin(2) = 2.0/(8.0)+dble(ik2-1)/dble(nk2)
    qin(2) = ( kshift(2) + dble( ik2 - 1 )) / dble( nk2 )
  do ik3=1,nk3
!  qin(3) = 3.0/(8.0*dble(nk3))+dble(ik3-1)/dble(nk3)
!   qin(3) = 0.5d0/dble(nk3) + dble( ik3 -1) / dble(  nk3 )
 ! qin(3) = 3.0/(8.0)+dble(ik3-1)/dble(nk3)
...
!  do j = brange(2) + overlap, ibh + overlap
     !open( unit=99, file='kdpprog', form='formatted', status='unknown' )
     !rewind 99
     !write ( 99, '(8i5)' ) ik1, ik2, ik3, 2 * nk1, 2 * nk2, 2 * nk3, j, ibh
     !close( unit=99 )
