...
program builder
  ...
  !
  integer :: i, j, ibl, ibh, nk1, nk2, nk3, ik1, ik2, ik3, it, nt, npt, ii, overlap, nspn, ispn
  !
  real( kind = kind( 1.0d0 ) ) :: celvol, pi, fr, fi, phse, su, pref, su2, spin_fac
  ...
  !
  real( kind = kind( 1.0d0 ) ), allocatable :: zr( :, : ), zi( :, : ), w( : ), w2( : ), muminusw( : )
  ...
  real( kind = kind( 1.0d0 ) ), allocatable :: wfr( : ), wfi( : ), wfp( :, : ), dmat( :, : ), wfpr( :, : ), wfpi( :, : )
  ...
  write(6,*) brange(:)
  !
  open(unit=99,file='nspn.ipt',form=f9,status='old')
  read(99,*) nspn
  close(99)
...
!!!!
  ...
  allocate(wfpr(npt,npt),wfpi(npt,npt))
  ...
  allocate( muminusw( ibl : ibh ) )
!  allocate( gre( npt, npt, 2 * nt ), gim( npt, npt, 2 * nt ) )
  allocate( gre( npt, npt, nt ), gim( npt, npt, nt ) )
  gre = 0; gim = 0!; dmat = 0
...
!
!  If we have no spin polarization, then multiply results by 2
  deallocate( wfr, wfi, wfpr, wfpi )
 spin_fac = 2.0 / dble( nspn )
 do ispn=1,nspn
  if( ispn .eq. 2 ) then
    close( enkfile )
    open(unit=enkfile,file='enkfile.2',form='formatted',status='old')
    rewind(enkfile)
  endif
  do ik1=1,nk1
     ...
     do ik2=1,nk2
        ...
        do ik3=1,nk3
           ...
           call omp_realu2( gtot, npt, ibl, ibh, brange, overlap, w, mu, efermi, zr( 1, ibl ), zi( 1, ibl ), gvec, bvec, posn, ure, uim, muminusw )
           do i = 1, npt
              phse = 0
              do j = 1, 3
...
!$OMP PARALLEL DO &
!$OMP PRIVATE( j, i, ii, it, x, denr, deni, iden2, fr, fi, wfr, wfi, wfpr, wfpi ) &
!$OMP SHARED( ibl, ibh, npt, nt, cosqr, sinqr, ure, uim, pref, s, t, muminusw) &
!$OMP REDUCTION(+: gre ) &
!$OMP REDUCTION(+: gim ) &
!$OMP SCHEDULE( STATIC ) &
!$OMP DEFAULT( NONE )
           do j = ibl, ibh 
              allocate( wfi ( npt ), wfr( npt ), wfpr( npt, npt ), wfpi( npt, npt ) )
              ...
              !close( unit=99 )
                ...
                ! At this step we are assuming no magnetism / time reversal problems?
                do i = 1, npt ! for each psi k, conjg. at -k.  hence, we are real!
                   do ii = 1, npt
!                      wfp( ii, i ) = pref * ( wfr( i ) * wfr( ii ) + wfi( i ) * wfi( ii ) )
 ! This might be wrong
!                      wfp_comp( ii, i ) = pref * ( wfr( i ) * wfr( ii ) + wfi( i ) * wfi( ii ) &
!                                                 + wfr( i ) * wfi( ii ) - wfi( i ) * wfr( ii ) )
                      wfpr( ii, i ) = pref * ( wfr( i ) * wfr( ii ) + wfi( i ) * wfi( ii ) )
                      wfpi( ii, i ) = pref * ( wfr( i ) * wfi( ii ) - wfi( i ) * wfr( ii ) )
                   end do
                end do
                ! dmat is worthless 
!                dmat( :, : ) = dmat( :, : ) + 2.0d0 * wfp( :, : )
...
!                   gim( :, :, it ) = gim( :, :, it ) + fi * wfp( :, : )
                   gre( :, :, it ) = gre( :, :, it ) + fr * wfpr( :, : ) - fi * wfpi( :, : )
                   gim( :, :, it ) = gim( :, :, it ) + fi * wfpr( :, : ) + fr * wfpi( :, : )
                end do
! 
...
111 continue
  ...
  do i = 1, npt
     ...
     do j = 1, npt
        ...
        su = spin_fac * 2.0 * su * s / pi     ! 2 for spin, 2 for Ry
        xirow( j ) = su
