...
module ocean_qe54_files
  ...
  subroutine qe54_universal2KptAndSpin( uni, ikpt, ispin )
...
!    if( mod( ikpt + (ispin-1)*product(kpts(:)), npool ) .eq. mypool ) then
  ...
  subroutine qe54_return_my_val_bands( nbands, ierr )
    integer, intent( out ) :: nbands
    integer, intent( inout ) :: ierr

    if( .not. is_init ) ierr = 1
    nbands = pool_val_nbands
  end subroutine qe54_return_my_val_bands

  subroutine qe54_return_my_con_bands( nbands, ierr )
    integer, intent( out ) :: nbands
    integer, intent( inout ) :: ierr

    if( .not. is_init ) ierr = 1
    nbands = pool_con_nbands
  end subroutine qe54_return_my_con_bands
...
!      energies(:,:,:) = energies(:,:,:) * 2.0_DP
  ...
  pure function qe54_gkvFile( ikpt, ispin, isConduction ) result( fileName )
    integer, intent ( in ) :: ikpt, ispin
    logical, intent( in ), optional :: isConduction
    character(len=512) :: fileName
    ...
    if( is_shift ) then
      if( is_split ) then  ! Different directories, k-point counting is correct!
        if( is_ ) then
          write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefixSplit ), 'K', ikpt, '/', 'gkvectors.dat'
        else
        ...
        ik = ( ikpt * 2 ) - 1
        if( is_ ) ik = ik + 1
        write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefix ), 'K', ik, '/', 'gkvectors.dat'
      endif
    else
      write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefix ), 'K', ikpt, '/', 'gkvectors.dat'
    endif
  ...
  pure function qe54_evcFile( ikpt, ispin, isConduction ) result( fileName )
    integer, intent ( in ) :: ikpt, ispin
    logical, intent( in ), optional :: isConduction
    character(len=512) :: fileName
    ...
    if( is_shift ) then
      if( is_split ) then  ! Different directories, k-point counting is correct!
        if( is_ ) then
          write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefixSplit ), 'K', ikpt, '/', trim(evcString)
        else
          write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefix ), 'K', ikpt, '/', trim(evcString)
        endif
      else  ! same directory, k-points are lies! 
        ik = ( ikpt * 2 ) - 1
        if( is_ ) ik = ik + 1
        write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefix ), 'K', ik, '/', trim(evcString)
      endif
    else
      write( fileName, '(a,a1,i5.5,a1,a)' ) trim( prefix ), 'K', ikpt, '/', trim(evcString)
    endif
...
11    continue
    ...
    pool_con_nbands = qe54_getConductionBandsForPoolID( pool_myid )
    pool_val_nbands = qe54_getValenceBandsForPoolID( pool_myid )
...
!    write(6,*) 'gvecs', pool_root, pool_comm
  ...
  subroutine qe54_get_ngvecs_at_kpt_split( ikpt, ispin, gvecs, ierr )
    use OCEAN_mpi
    integer, intent( in ) :: ikpt, ispin
...
111 continue
  ...
  end subroutine qe54_get_ngvecs_at_kpt_split

  subroutine qe54_read_at_kpt_split( ikpt, ispin, valNGvecs, conNGvecs, valBands, conBands, &
    ...
    !
    if( qe54_getValenceBandsForPoolID( pool_myid ) .ne. valBands ) then
      write(1000+myid, * ) 'Valence band mismatch:', qe54_getValenceBandsForPoolID( pool_myid ), valBands
      ierr = 1
      return
    endif
    if( qe54_getConductionBandsForPoolID( pool_myid ) .ne. conBands ) then
      write(1000+myid, * ) 'Conduction band mismatch:', qe54_getConductionBandsForPoolID( pool_myid ), conBands
      ierr = 2
      return
    endif
  ...
  end subroutine qe54_read_at_kpt_split
  ...
  subroutine read_at_kpt_split( ikpt, ispin, valNGvecs, conNGvecs, valBands, conBands, &
    ...
    if( pool_myid .eq. pool_root ) then
      ...
      ! use the same buffer system as for qe54_read_at_kpt, but make buffers the max of conduction/valence
        ...
        if( id .eq. pool_myid ) then
          ...
          do i = 1, nbands_to_send
            if( start_band + i - 1 .le. overlapBands ) then
              ...
              read(99)
              read(99)
              read( 99 ) crap, conUofG( 1:test_gvec, i )
              read(99)
              read(99)
            endif
          enddo
