...
!  REAL(DP), ALLOCATABLE :: b( : )
  ...
  subroutine OCEAN_haydock_nonHerm_do( sys, hay_vec, ierr, hflag )
...
!      endif
     ...
     call OCEAN_action_h1(sys, inter_scale, psi, new_psi, ierr, .false., hflag)
         if( ierr .ne. 0 ) return
      ...
      !call OCEAN_xact( sys, inter_scale, back_psi, back_new_psi, ierr, .true. )
        call OCEAN_action_h1(sys, inter_scale, back_psi, back_new_psi, ierr,&
        .true.,hflag)
        if( ierr .ne. 0 ) return
...
!    integer, intent(in):: hflag(6)
  ...
  subroutine OCEAN_haydock_Herm_do( sys, hay_vec, restartBSE, newEps, ierr, hflag )
    use AI_kinds
    use OCEAN_mpi
    use OCEAN_system
    use OCEAN_energies
...
#if( 0 )
!AK - This subroutine is not used, no new modifications were made to it. 
