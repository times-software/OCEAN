...
!      call tmelsCalcUmklapp
  ...
  subroutine tmelsCalc( ikpt, ispin, nValBands, nConBands, valGvecs, conGvecs, &
    ...
    if( ikpt .lt. 1 ) then
      ...
      call MPI_WRITE_ALL( FH, valUofG, i, MPI_DOUBLE_COMPLEX, MPI_STATUS_IGNORE, ierr )
      return
    endif
...
11      continue
    ...
    call MPI_WRITE_ALL( FH, tmels, i MPI_DOUBLE_COMPLEX, MPI_STATUS_IGNORE, ierr )
  ...
  subroutine ocean_tmels_open( nValBands, nConBands, myConBands, myConBandStart, myNK, myKStart, kStride, ierr )
    ...
    integer, intent( in ) :: nValBands, nConBands, myConBands, myConBandStart, myKStart, kStride
    ...
    call MPI_FILE_OPEN( comm, "ptmels", fflags, MPI_INFO_NULL, FH, ierr )
    if( ierr .ne. 0 ) return
    ...
    offset = ( myConBandStart - 1 ) * nValBands &
           + ( myKStart - 1 ) * int( nValBands, MPI_OFFSET_KIND ) * int( nConBands, MPI_OFFSET_KIND )
