subroutine melwriter( etot, rel, alfa, nr, r, dr, r2, dl, njrc, vi, zorig, xntot, nel, iuflag, cq, isuse )
  ...
  double precision :: su, dummy, err, x1, x2
  double precision, external :: efrac
  !
  double precision, allocatable :: c( : , : )
  double precision, allocatable :: meltrue( :, : ), melfake( :, : ), melval( :, : ), meltmp( : )
     ...
     do i1 = 1, nt
        do i2 = 1, nt
           do lc = lcmin, lcmax, 2
              x1 = efake( lc, i1, i2 )
              x2 = etrue( lc, i1, i2 )
              write ( 99, '(2x,2i5,4(2x,1e15.8))' ) i1, i2, ev( i1 ), ev( i2 ), x1, x2
           end do
        end do
        write ( 99, * )
     end do
     ...
     do j = 0, 4
        ...
        case( 0 )
           ...
           if ( arg .gt. 0.001d0 ) then
              fcn = ( sin( arg ) / arg - 1.0d0 ) / q
           end if
        case( 1 )
           fcn = 3.0d0 * ( arg / 3.0d0 - arg ** 3 / 30.0d0 ) / q
           if ( arg .gt. 0.001d0 ) then
              fcn = 3.0d0 * ( sin( arg ) / arg ** 2 - cos( arg ) / arg ) / q
           end if
        case( 2 )
           fcn = 5.0d0 * ( arg ** 2 / 15.0d0 - arg ** 4 / 210.0d0 ) / q
           if ( arg .gt. 0.001d0 ) then
              fcn = 5.0d0 * ( ( 3.0d0 / arg ** 3 - 1.0d0 / arg ) * sin( arg ) - 3.0d0 / arg ** 2 * cos( arg ) ) / q
           end if
        end select
