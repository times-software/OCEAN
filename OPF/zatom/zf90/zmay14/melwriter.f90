subroutine melwriter(etot,rel,alfa,nr,r,dr,r2,dl,njrc,vi,zorig,xntot,nel, iuflag,cq,isuse)
  ...
  do l = lmin, lmax
     ...
     call sdump( l, .true., nelmax, nl, nr, r, pheps )
     do i = 1, ne
        do j = 1, ne
           do k = 1, irc
     ...
     call sdump( l, .false., nelmax, nl( nco + 1 ), nr, r, pheae )
     ...
     do i = 1, ne
        ...
        case( 'longit' )
           call longit( nr, r, dl, phespc, pheae( 1, lev ), tmp, l, q )
        end select
     ...
     case( 'longit' )
        if ( ( l .ge. lmin ) .and. ( l .le. lmax ) ) write ( melfile, '(4(2x,1e15.8))' ) melval
     end select
     !
     call abinitio(etot,rel,alfa,nr,r,dr,r2,dl, &
...
!
subroutine longit( nr, r, dl, ph1, ph2, mel, lv, q )
  implicit none
  !
  integer :: nr, lv
  ...
  do i = 1, nr - 4, 4
     do j = 0, 4
        ...
        select case( lv )
        case( 0 )
           fcn = ( -( arg ** 2 / 6.0d0 ) + arg ** 4 / 120.0d0 ) / q
           if ( arg .gt. 0.001d0 ) fcn = ( sin( arg ) / arg - 1.0d0 ) / q
        case( 1 )
           fcn = 3.0d0 * ( arg / 3.0d0 - arg ** 3 / 30.0d0 ) / q
           if ( arg .gt. 0.001d0 ) fcn = 3.0d0 * ( sin( arg ) / arg ** 2 - cos( arg ) / arg ) / q
        case( 2 )
           fcn = 5.0d0 * ( arg ** 2 / 15.0d0 - arg ** 4 / 210.0d0 ) / q
           if ( arg .gt. 0.001d0 ) fcn = 5.0d0 * ( ( 3.0d0 / arg ** 3 - 1.0d0 / arg ) * sin( arg ) - 3.0d0 / arg ** 2 * cos( arg ) ) / q
        end select
        f( j ) = ph1( ii ) * ph2( ii ) * r( ii ) * fcn
     end do
     mel = mel + 14.d0 * ( f( 0 ) + f( 4 ) )
     mel = mel + 64.d0 * ( f( 1 ) + f( 3 ) )
     mel = mel + 24.d0 * f( 2 )
