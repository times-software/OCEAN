subroutine getfg( nr, dl, r, nel, nl, phe )
  ...
  !
  integer :: ic, i2, i3, i, j, k, lv, lc, ll, lh, kk
  ...
  do lv = ll, lh
     ...
     phv( :, : ) = 0
     write ( filnam3, '(1a2,1i1)' ) 'ae', lv
     open( unit=99, file=filnam3, form='formatted', status='unknown' )
     rewind 99
     do i = 1, nr
        read ( 99, * ) dum, temp( : )
        do j = 1, np( lv )
     ...
     do k = 0, 2 * max( lv, lc )
        ...
        if ( hart ) then
           write ( filnam5, '(1a2,3i1)' ) 'fk', lc, lv, k
           open( unit=99, file=filnam5, form='formatted', status='unknown' )
           rewind 99
           do i2 = 1, np( lv )
        ...
        if ( fock ) then
           write ( filnam5, '(1a2,3i1)' ) 'gk', lc, lv, k
           open( unit=99, file=filnam5, form='formatted', status='unknown' )
           rewind 99
           do i2 = 1, np( lv )
