...
Several effects can be probed with RIXS, including the phonons and spin-density waves, but \program{ocean} is a BSE code, and, therefore, it is only capable of exploring the excitonic, band-structure contributions to RIXS spectra. 
After a brief overview of the theory we will walk through an example looking at the carbon K edge in diamond.
In addition to the regular input file the diamond example has a shell script \file{diamond.sh}. 
You will need to edit this script to include the path of the \program{ocean} executables.

\subsection{RIXS theory}
Before embarking on RIXS calculations we implore you to read the excellent reviews by Kotani and Shin (Rev.\ Mod.\ Phys.\ {\bf 73}, 203, 2001) and Ament {\it et al.} (Rev.\ Mod.\ Phys.\ {\bf 83}, 705, 2011). With \program{ocean} we are able to calculate what Ament {\it et al.} refer to as direct RIXS. To be filled in later.

\subsection{Method}

The way we go about RIXS calculations with \program{ocean} is a two step process: a set of core-level calculations followed by a valence one. 
...
This can be approximated using iterative techniques, such as the generalized minimal residual method used in \program{ocean} (Saad and Schultz, 1987). 
See section \ref{GMRES-section} for a summary and reference to the parameters that control this approximation within \program{ocean}.
...
The Hamiltonian in Eq.~\ref{rixs-method4} is acting on a space containing a valence hole and a conduction band electron, and is approximated in \program{ocean} using the valence BSE solver, originally released as \program{ai2nbse}. 
...
In the example directory is the script \file{diamond.sh} which will run all the steps necessary for the RIXS calculation of diamond.
This script can either be run directly (on a workstation or interactive session) or pasted into a batch script,
but you'll need to edit the variable ``OCEAN\_BIN'' to point to your \program{ocean} installation directory.
As remarked in the previous section the calculation is broken up into several steps. 
The provided \file{diamond.sh} script can be used as a template for your own RIXS runs.
...
As with any tutorial feel free to start it before reading the section. In this case run it using the script \file{diamond.sh} which may require editing to specify the path to your \program{ocean} installation. 
There are a couple of differences in this input file \file{diamond.in} from some of the other tutorials that should be noted. 
...
\begin{figure}
\center
\includegraphics[angle=270,width=4.5in]{diamond_xas.eps}
...
The second (and third) steps are both run by the helper code \program{rixs.pl}. 
We now much determine the excitonic states at each incident photon energy (and polarization) which requires switching the algorithm in the BSE code. This is done via the line in \program{diamond.sh} that says ``echo gmres $>$ Common/cnbse.solver'', which switches us to using the GMRES method. 
There are two ways to determine which energies to use for the incoming photons. 
In the diamond example we make use of `cnbse.gmres.erange' which allows us to set the energy range and step size. The units are eVs and the range is given with respect to the same energy axis of the \file{absspct} files from the previous absorption calculation (this is why we do that first).
In \program{diamond.sh} the line `echo 0 30 2 $>$ Common/cnbse.gmres.erange' tells the code to start at 0 and calculate every 2~eV until it hits 30.
The last input needed to determine the excitons is the description of the incoming photon. By default the code will use every available photon file, but by using ``photon\_in'' we are able to tell it to only use the first one \file{photon1}.

After the core-level BSE has run a new set of \file{absspct} files has been created, but these have a slightly different format. 
You should plot \file{CNBSE/absspct\_C\_.0001\_1s\_01} using columns 1 and 3 and \file{RIXS/absspct\_C\_.0001\_1s\_01} using columns 2 and 4 (see Fig.~\ref{diamond_xas_plot}). 
...
More importantly, the \file{RIXS} directory is now full of \file{echamp\_C\_} files. These contain the information about the excitons $y_{\hat{\epsilon},\alpha}(\omega)$. 
...
The code also has generated \file{cksv} files which contain the transition information for a valence-band electron to fill the core hole $\langle v \vert \hat{d}^\dagger_{\mathbf{k},\hat{\epsilon}} \vert \alpha \rangle $. 
...
The naming scheme is then something like \file{rxsspct\_C\_.0000\_1s\_01.08.02}, where we are looking at the carbon 1{\it s} edges using the 1st photon file as our incoming beam, at the 8th energy step, and  the 2nd photon file as the outgoing x-ray. {(\it N.b.} we are planning on getting rid of the unused `0000' which for \file{absspct} or \file{xesspct} files denote the atomic site.)
The format of the \file{rxsspct} files follows that of the valence BSE output. 
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[angle=270,width=0.9\linewidth]{diamond_rixs_loss.eps}
 % \caption{RIXS of diamond as a function of energy loss}
  \label{fig:sub1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[angle=270,width=.9\linewidth]{diamond_rixs_em.eps}
 % \caption{RIXS of diamond as a function of emission energy}
  \label{fig:sub2}
\end{subfigure}
...
\begin{description}
 ...
 \item[\textbf{Optional Cards:}] 
    \htmlref{PARA\_PREFIX}{card:paraprefix}, \htmlref{SER\_PREFIX}{card:serprefix},
    \htmlref{DFT}{card:dft}, 
...
\item[\large\textbf{Structural information}]\dotfill\
  ...
  \item[\textbf{Required Cards:}] 
    ...
    \item[\textbf{Optional Cards:}] 
       \htmlref{NSPIN}{card:nspin},
       \htmlref{SMAG}{card:smag},
       \htmlref{LDAU}{card:ldau}, and
       \htmlref{ZYMB}{card:zsymb}
  \end{description}
...
\item[\large\textbf{Non-Structural DFT parameters}]\dotfill\
  ...
  \item[\textbf{Optional Cards:}] 
    ...
    \htmlref{NSTEP}{card:nstep},
    \htmlref{SCREEN.NKPT}{card:screen.nkpt},
    \htmlref{ABPAD}{card:abpad}, \\
    \htmlref{DFT\_ENERGY\_RANGE}{card:dft_energy_range},
    \htmlref{SCREEN\_ENERGY\_RANGE}{card:screen_energy_range},
    \htmlref{K0}{card:k0}, 
    and \htmlref{DEGAUSS}{card:degauss}
  \end{description}
...
\item[\large\textbf{BSE Parameters}]\dotfill\
  ...
  \item[\textbf{Recommended Cards:}]   
    \htmlref{CNBSE.XMESH}{card:cnbse.xmesh}, \htmlref{CNBSE.MODE}{card:cnbse.mode},
     \htmlref{CNBSE.NITER}{card:niter},
    \htmlref{METAL}{card:metal},
    \htmlref{SPIN-ORBIT}{card:spin-orbit}, 
    \htmlref{PHOTON\_Q}{card:photonq}, 
%    \htmlref{CKS.NORMAL}{card:cks.normal},
      \htmlref{CNBSE.STRENGTH}{card:strength},
      \htmlref{CNBSE.SOLVER}{card:cnbse-solver}, \\
      \htmlref{CNBSE.GMRES.ELIST}{card:gmres-elist}, 
      \htmlref{CNBSE.GMRES.ERANGE}{card:gmres-erange},
      \htmlref{CNBSE.GMRES.NLOOP}{card:gmres-nloop},\\
      \htmlref{CNBSE.GMRES.GPRC}{card:gmres-gprc}, 
      and \htmlref{CNBSE.GMRES.FFFF}{card:gmres-ffff}
  \end{description}
...
\program{mpiexec}, \program{srun}, \program{aprun}, \&c. 
The \program{ocean} scripts are designed to try and parse this to figure out how many processors you are trying to run across, e.g., `mpirun -n 12' will be parsed to tell \program{ocean} to prepare DFT runs across 12 cores. 
...
On some systems \program{cut3d} will hang when launched on multiple processors, but must be launched with the parallel job launcher from above, i.e., `mpirun -n 1'.
\end{Card}
...
Controls the initial magnetism for the DFT calculations. See the hematite example and the \program{abinit} or \program{QuantumESPRESSO} documentation for more information.
\end{Card}
...
Each element listed in \htmlref{ZNUCL}{card:znucl} also needs a symbol when running using \program{QuantumESPRESSO}.
...
\item[\texttt{occopt} = 3]\hfill\\ States are all doubly degenerate but can have fractional occupations depending on fermi level. Suitable for metals. For \program{Quantum Espresso} this is equivalent to OCCUPATIONS = `smearing' and smearing = `fermi-dirac'.
\end{description}
\end{Card}
...
The maximum number of iterations the DFT code attempt. If convergence is not reached after \texttt{nstep} iterations then \program{abinit} will continue with a silent warning and \program{QuantumESPRESSO} will fail.
The default is 50.
\end{Card} 
...
\end{equation}
where $N_k$ is given by \htmlref{nkpt}.
...
Each edge entry consists of 3 integers. When the first is greater than 0 it denotes the {\bf index} of the atom (in the order set by \htmlref{XRED}{card:xred}), and when it is less than zero it sets the (negative) $Z$, allowing you to specify {\bf all} of a given element.
...
The spin-orbit splitting between, e.g., the L$_2$ and L$_3$ edges, is automatically calculated within \program{ocean}, but can be overridden by setting SPIN-ORBIT $ \ge 0$. 
...
Note, if both this and \htmlref{CNBSE.GMRES.ERANGE}{card:gmres-erange} are specified then \htmlref{CNBSE.GMRES.ERANGE}{card:gmres-erange} will be used.
\end{Card}
...
This is especially useful for reproducing RIXS planes, in contrast to \htmlref{CNBSE.GMRES.ELIST}{card:gmres-elist}  where the energy points can be arbitrarily spaced.
If both CNBSE.GMRES.ERANGE and \htmlref{CNBSE.GMRES.ELIST}{card:gmres-elist} are specified this will be used.
\end{Card}

\begin{Card}{CNBSE.GMRES.NLOOP}{cnbse.gmres.nloop}{Optional}{gmres-nloop}
The version of the GMRES algorithm implemented in \program{ocean} will grow to be a subspace of no larger than NLOOP before it is restarted. By default this is set to 80, and should probably not be changed without careful timing tests.
\end{Card}

\begin{Card}{CNBSE.GMRES.GPRC}{cnbse.gmres.gprc}{Optional}{gmres-gprc}
The version of the GMRES algorithm implemented in \program{ocean} uses the one-electron energies as a pre-conditioner with a Lorentzian broadening set by GPRC (in Ha.). By default this is 0.5~Ha.\ which should be reasonable for K edges.
 \end{Card}
