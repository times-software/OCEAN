...
\vspace*{\stretch{1}}
 ...
 \noindent This document is copyright \copyright\ 2010-2016 \program{ocean} collaboration, University of Washington. Following conventions of the FEFF documentation
...
\end{itemize}
The two should match very closely, but with the quality settings in the input there may well be differences between the DFT codes (see figure \ref{STO_plot}). The 3{\it d} transition metals highlight the breakdown of a single-particle picture for x-ray absorption. A quick count shows that the intensity ratio between the L$_2$ and the L$_3$ edges should be 2:1 from the initial occupations of the 2$p_{3/2}$ and 2$p_{1/2}$. Instead for titanium we see a ratio of less than 1. The spin-orbit splitting of the 2{\it p} orbitals is not diagonal in the same basis as the conduction band $d$ states, and so trying to move backward from the spectrum to the ground-state density of states (for either the 2$p$ or 3$d$ levels) can be difficult, but the BSE approach in \program{ocean} makes it easy.
...
\item Cutoff radius: The OPF cutoff radius is in Bohr and sets the sphere in which the local basis is calculated. Convergence of this must be checked. 
...
The \file{pr\#} files contain the projectors on a real-space grid by $l$. The projectors are, by definition, 0 outside of the OPF cut-off radius. They are kept in the \file{zdiag} directory.
