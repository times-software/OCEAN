...
!
  ...
  integer :: sizer, xtics, ytics, ztics, scaled(3)
...
!  allocate(psiGr(ngfft(1,1):ngfft(2,1), ngfft(1,2):ngfft(2,2), ngfft(1,3):ngfft(2,3) ),  &
  allocate( psi(2*ngfft(1,1):2*ngfft(2,1), 2*ngfft(1,2):2*ngfft(2,2), 2*ngfft(1,3):2*ngfft(2,3) ),  &
...
!       
        ...
        do xiter=2*ngfft(1,1),2*ngfft(2,1)
          do yiter=2*ngfft(1,2),2*ngfft(2,2)
            do ziter=2*ngfft(1,3),2*ngfft(2,3)
!!!              do giter=1,ng
...
!!                enddo
                ...
                phase = (qraw(1)) * (xiter / gsize(1) - tau(1)) &
                      + (qraw(2)) * (yiter / gsize(2) - tau(2)) & !(redcoord(2) - tau(2)) &
                      + (qraw(3)) * (ziter / gsize(3) - tau(3))
                phase = phase * 2 * pi
                ...
                scaled(2) = yiter
                scaled(3) = ziter
                if (xiter .lt. ngfft(1,1) ) then
                  scaled(1) = xiter+gsize(1)
                elseif (xiter .gt. ngfft(2,1) ) then
                  scaled(1) = xiter-gsize(1)
                endif
                if (yiter .lt. ngfft(1,2) ) then
                  scaled(2) = yiter+gsize(2)
                elseif (yiter .gt. ngfft(2,2) ) then
                  scaled(2) = yiter-gsize(2)
                endif
                if (ziter .lt. ngfft(1,3) ) then
                  scaled(3) = ziter+gsize(3)
                elseif (ziter .gt. ngfft(2,3) ) then
                  scaled(3) = ziter-gsize(3)
                endif

                psi(xiter,yiter,ziter) = psi(xiter,yiter,ziter) + phi(scaled(1), scaled(2), scaled(3))*prefac
...
!!  deallocate(COMM)
!
  redcoord(:) = -(avecs(1,:) + avecs(2,:) + avecs(3,:))
  write(6,*) 'writing out wf'
  open(unit=99,file='output',form='formatted',status='unknown')
  write(99,*) 'dunno'
  write(99,*) 'bah'
  write(99,'(1i5,3(1x,1e15.8))') 0, redcoord(:)
  write(99,'(1i5,3(1x,1e15.8))') gsize(1), 2*avecs(1,:)/gsize(1)
  write(99,'(1i5,3(1x,1e15.8))') gsize(2), 2*avecs(2,:)/gsize(2)
  write(99,'(1i5,3(1x,1e15.8))') gsize(3), 2*avecs(3,:)/gsize(2) !ztics*2+1, 0, 0, convcell(3)/dble(ztics*2+1) !gsize(3), avecs(3,:)/gsize(2)
  do xiter=2*ngfft(1,1),2*ngfft(2,1),2
    do yiter=2*ngfft(1,2),2*ngfft(2,2),2
      do ziter=2*ngfft(1,3),2*ngfft(2,3),2
