...
!
      integer irval,nrval,ileft,iright,iu
...
c
      integer  chunk,kk,kklocal
      external chunk
c
      ir=13
      iw=14
      open ( unit=ir, file='sepipt',
     &       form='formatted', status='unknown' )
      rewind ir
      open ( unit=iw, file='sepopt',
     &       form='formatted', status='unknown' )
      rewind iw
      read (ir,*) nk
      ...
      read (5,*) klocal,nbl,nbh,xl,xh,nx
      if (nbh.gt.nbm) stop 'increase nbm in kleinman'
      if (klocal.gt.0) then
        nkm1=nk-1
        kklocal=chunk(nl(klocal),nj2(klocal))
      else
        stop 'other stuff not presently supported'
      end if
      write (iw,722) nkm1,nr,r1,r2
  722 format(1x,2i8,2(1x,1f20.15))
c
c                      loop over l,j channels
c
      do k=1,nk
        l =nl (k)
        j2=nj2(k)
        ...
        if ( k .eq. klocal ) then
          open( unit=99, file='llocal', form='formatted',
     &          status='unknown' )
          rewind 99
          write ( 99, '(2x,1i3)' ) l
          close( unit=99 )
...
c
          ...
          write ( 6, * ) 'results output'
          if (nx.ge.0) then
            call kbdiag(r1,r2,nr,jrt,r0,nx,xl,xh,nbl,nbh,k,l,
     &                  ph0,ph1,ph2,ps0,ps1,ps2,
     &                  q,vnl,vps(1,kklocal),orb(1,k))
          end if
          write ( 6, * ) 'diagnostics complete'
        end if
      end do
c
c
c  output s-,p-,d-channel addenda for multiple-scattering calcs
c
      iu=16
      open( unit=iu, file='sep.ms',
     &      form='formatted', status='unknown' )
      rewind iu
      write (iu,9022) delta,nrval,' UNITS = BOHRS and RYDBERGS '
 9022 format(1x,1f20.10,1i5,1a28)
      do irval=1,nrval
        rr=delta*dble(irval-1)
        write (iu,9032) irval,nrval,rr,
     &                  vadds(irval),vaddp(irval),vaddd(irval),
     &                  vaddf(irval)
 9032   format(1x,2i5,1f10.5,5x,4(1x,1f15.10))
      end do
c
...
c---------------------------------------------------------------------
     ...
     &                  ph0,ph1,ph2,ps0,ps1,ps2,q,vnl,vlc,orb)
      implicit none
c
...
c
      call get(nr2,ph0,ph0,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph0,ph1,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph0,ph2,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph1,ph0,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph1,ph1,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph1,ph2,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph2,ph0,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph2,ph1,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      call get(nr2,ph2,ph2,q,ps0,ps1,ps2,vnl,r1,r2,tmp)
      do i=1,nr2
        vloc(i)=vlc(i)+orb(i)
      end do
      do ix=0,nx
        ...
        do mode=1,2
          ...
          write (iu,1912) (xx,reval(i),i=1,nbh)
 1912     format(1x,2f18.6)
        end do
      end do
      return
      end
...
c---------------------------------------------------------------------
      ...
      implicit none
      double precision w(9),w00,w01,w02,w11,w12,w22
      w(1)=w00
      w(2)=w01
      w(3)=w02
      w(4)=w01
...
c---------------------------------------------------------------------
      ...
      implicit none
      integer l,is
      double precision xget,q,x,qa,xa,qb,xb,r0,qq,xx
      external xget
CD    if ( l .eq. 3 ) write ( 6, * ) 'l=3, way in'
      is=10
  100 if (is.gt.0) qq=qa+(qb-qa)*(xa-x)/(xa-xb)
      if (is.le.0) qq=0.5d0*(qa+qb)
      is=is-1
      if (is.eq.-10) is=10
      xx=xget(qq,r0,l)
      if ( dabs( xx - x ) .gt. 0.000001d0 ) then
         if ( ( xx - x ) * ( xa - x ) .gt. 0.d0 ) then
            qa = qq
            xa = xx
         else
...
c---------------------------------------------------------------------
      ...
      implicit none
      double precision olap,jlof,prec,temp,q1,q2,r0,xj,dr,r
      integer np,ip,l
      external jlof
      prec=0.000001d0
      temp=0.d0
...
c---------------------------------------------------------------------
     ...
     &        +d1*(q(4)*ps0(j)+q(5)*ps1(j)+q(6)*ps2(j))
     &        +d2*(q(7)*ps0(j)+q(8)*ps1(j)+q(9)*ps2(j))
        v2(j)=vloc(j)*psi(j)
        v3(j)=vnl (j)*psi(j)
      end do
      return
...
c---------------------------------------------------------------------
     ...
     &      +pl1*(q(4)*pr0+q(5)*pr1+q(6)*pr2)
     &      +pl2*(q(7)*pr0+q(8)*pr1+q(9)*pr2)
      write (6,222) matsl,matnl,matsl-matnl
 222  format(1x,3(1x,1f20.10))
      return
      end
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
      ...
      write (iw,1422) l,j2
 1422 format(1x,2i5)
      write (iw,1432) (q(i),i=1,9)
      write (iw,1432) (ps0(i),ps1(i),ps2(i),i=1,nr)
 1432 format(1x,3(2x,1f20.10))
      return
      end
