...
c
      ...
      open( unit=99, file='vlocal', form='formatted', status='unknown' )
      rewind 99
      do j = 1, nrp
         write ( 99, '(2(1x,1e15.8))' ) r( j ), vrl( j )
      end do
      close( unit=99 )
c     do 120 j=1,nrp
...
c120  continue
       ...
       call qlocal(delql,nql,nrp,r,a,b,c,vrl,vql,vql0,zizv)
...
c
       ...
       write ( 7, '(2x,1f10.5,2(1x,1i5),3(1x,1e20.12))' )
     &   zizv, nql, nqnl, delql, delqnl, vql0
       write ( 7, '(4(1x,1e18.10))' ) ( vql( j ), j = 1, nql )
       write ( 7, '(4(1x,1e18.10))' ) ( dcq( j ), j = 1, nql )
       write ( 7, '(4(1x,1e18.10))' ) ( dvq( j ), j = 1, nql )
c
       close( unit=7 )
       open( unit=99, file='localft', form='formatted', 
     &   status='unknown' )
       rewind 99
       do j = 1, nql 
          write ( 99, '(2(1x,1e15.8))' ) dble( j ) * delql, vql( j )
       end do
       close( unit=99 )
c
...
c
       write ( 6, * ) 'elsft terminus achieved'
       end
c
c---------------------------------------------------------------------
c
       subroutine qlocal(delql,nql,nrp,r,a,b,c,vrl,vql,vql0,zizv)
       implicit double precision (a-h,o-z)
c
