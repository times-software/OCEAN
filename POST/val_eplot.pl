...
print "Plot Hole(1)  or Electron(2) part? Enter 1 or 2:\n";
<STDIN> =~ m/(\d)/ or die;
my $ehflag=$1;
open OUT, ">ehflag.ipt" or die;
print OUT "$ehflag\n";
close OUT;

print "Specify fractional coordinate to fix hole or electron\n:";
print "Default is {0.5, 0.5, 0.5}\n";
my @coor;
my $line = <STDIN> ;
$line =~ s/\s+/:/g;
if ( $line =~ m/([-]?\d+\.\d+[Ee]?[-+]?(\d+)?):([-]?\d+\.\d+[Ee]?[-+]?(\d+)?):([-]?\d+\.\d+[Ee]?[-+]?(\d+)?)/ ) 
{
    @coor=split(/:/,$line);
...
{
    $coor[0]=0.5; $coor[1]=0.5; $coor[2]=0.5;  
}
print "Using electron/hole coordinate: @coor \n";
open OUT, ">ehcoor.ipt" or die;
print OUT "@coor\n";
close OUT;
...
system( "$ENV{'OCEAN_BIN'}/val_exciton_plot.x") == 0 or die "Failed to run exciton_plot.x correctly\n";
