!/local/hadley/cvs_ai2nbse/AI2NBSE/Src/NBSE/zbridge/zf77/spectrum.f
      ...
      integer, parameter :: eps1 = 41, eps2 = 42, sedat = 47
      ...
      double precision reeps, imeps, indref, indabs, rad, delta, gamse
      double precision ref, theta, pi, efact
...
C     read ( stdin, * ) n
     ...
     &       ( dble( nk ) * vol ) / pi
      open( unit=eps1, file='eps1_SE', form=f9, status=u7 )
      open( unit=eps2, file='eps2_SE', form=f9, status=u7 )
      open( unit=loss, file='loss_SE', form=f9, status=u7 )
      open( unit=refl, file='refl_SE', form=f9, status=u7 )
      open( unit=inds, file='inds_SE', form=f9, status=u7 )
      open( unit=sedat,file='SE.dat',status='old' )
      !mpp Aug. 2008
      !open opcons.dat and write a short header
      open( unit=opcons, file='opcons_SE.dat', form=f9, status=u7 )
     ...
     &"               epsinv"
      ...
      de = ( eh - el ) / dble( ne )
      do ie = 0, ne
        read(sedat,*) delta, delta, delta, gamse
        ere = el + de * dble( ie )
        e = dcmplx( ere, gam + gamse )

        arg = ( ere - a( n )) ** 2 - 4.d0 * b( n + 1 ) ** 2
!	Josh: Changing ere to e everywhere
!        arg = ( e - a( n )) ** 2 - 4.d0 * b( n + 1 ) ** 2
        arg = cdsqrt( arg )
...
!        rm = 0.5d0 * ( e - a( n ) - arg )
        ...
        be  = - e - delta - a( n ) - r
        do i = n, 0, - 1
          al =    e - delta - a( i ) - b( i + 1 ) ** 2 / al
          be  = - e - delta - a( i ) - b( i + 1 ) ** 2 / be
        end do
